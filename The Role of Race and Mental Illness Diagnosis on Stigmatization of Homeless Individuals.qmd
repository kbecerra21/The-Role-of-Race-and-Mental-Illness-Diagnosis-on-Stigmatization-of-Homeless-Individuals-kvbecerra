...
```
## Main effects of race and diagnosis on social interaction
Figure 1 shows the effect of mental illness diagnosis on social distance stigma. The mean stigma scores for each diagnosis are as follows:

- **No Diagnosis**: `r data$mean_stigma[1]` (SD = `r data$sd_stigma[1]`)
- **Schizophrenia**: `r data$mean_stigma[2]` (SD = `r data$sd_stigma[2]`)
- **Substance Use Disorder**: `r data$mean_stigma[3]` (SD = `r data$sd_stigma[3]`)

The results indicate that individuals with schizophrenia (M = `r data$mean_stigma[2]`) and substance use disorder (M = `r data$mean_stigma[3]`) experience higher levels of social distance stigma compared to those with no diagnosis (M = `r data$mean_stigma[1]`). However, these differences were not statistically significant.
