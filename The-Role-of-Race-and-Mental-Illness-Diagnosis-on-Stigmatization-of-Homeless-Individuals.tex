...
The results of the 2x3 between-subjects ANOVA examining the effects of
race and mental illness diagnosis on social distance stigma are
presented below.

\subsection{Summary of ANOVA Results}\label{summary-of-anova-results}

The table below summarizes the results of the ANOVA:

\section{Extracting ANOVA summary for the
table}\label{extracting-anova-summary-for-the-table}

anova\_summary \%\textgreater\% as.data.frame() \%\textgreater\%
rownames\_to\_column(var = ``Source'') \%\textgreater\% rename(DF = Df,
SS = \texttt{Sum\ Sq}, MS = \texttt{Mean\ Sq}, F\_Value =
\texttt{F\ value}, p\_value = \texttt{Pr(\textgreater{}F)})
\%\textgreater\% gt() \%\textgreater\% fmt\_number(columns = c(SS, MS,
F\_Value, p\_value), decimals = 3) \%\textgreater\% tab\_header( title =
``Table 1'', subtitle = ``ANOVA Results for Social Distance Stigma'' )
\%\textgreater\% cols\_label( Source = ``Factor'', DF = ``df'', SS =
``SS'', MS = ``MS'', F\_Value = ``F'', p\_value = ``p'' )
\%\textgreater\% fmt\_scientific(columns = p\_value, decimals = 3)
\%\textgreater\% tab\_source\_note(``Note. SS = Sum of Squares; MS =
Mean Square; p-values reported in APA format.'')
